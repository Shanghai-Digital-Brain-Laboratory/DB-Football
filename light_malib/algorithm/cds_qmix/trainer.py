...
class CDS_QMixTrainer(Trainer):
    def __init__(self, tid):
        super().__init__(tid)
        self.id=tid
        self._loss = CDS_QMixLoss()
    ...
    def optimize(self, batch):      #[batch, 1, num_agent, feat_dim]
        ...
        data_generator_fn = functools.partial(
            dummy_data_generator, batch, num_mini_batch, 'cpu', shuffle=False
        )
    ...
    def reset(self, policies, training_config):
        ...
        if self.loss.mixer is None:
            self.loss.set_mixer(QMixer(qmix_config))
            self.loss.set_mixer_target(QMixer(qmix_config))

            with torch.no_grad():
                for target_param, param in zip(self.loss.mixer_target.parameters(), self.loss.mixer.parameters()):
                    target_param.data.copy_(param.data)

        super(CDS_QMixTrainer, self).reset(policies, training_config)
